...
The `incubating` and `stable` allocator variants used by older scheduler
versions cannot evaluate this constraint. When those variants encounter a pool
that contains at least one device with a non-empty `compatibilityGroups` field,
they skip the entire pool rather than risk a wrong allocation decision. Devices
in pools without any `compatibilityGroups` declarations are unaffected.
...
Device compatibility groups is an *alpha feature* and only enabled when both the
...
and the
[`DRAPartitionableDevices` feature gate](/docs/reference/command-line-tools-reference/feature-gates/#DRAPartitionableDevices)
are enabled in the kube-apiserver and kube-scheduler.
